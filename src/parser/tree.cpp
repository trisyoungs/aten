...
Tree::Tree(const char *name, const char *commands)
{
	...
	// Initialise
	initialise();
	cmdparser.generate(this, commands);
}
...
bool Tree::execute(ReturnValue &rv)
{
	...
	{
 		//printf("Executing tree statement %p...\n", ri->item);
// 		ri->item->nodePrint(1);
...
// 			msg.print(Messenger::Parse, "Execution of tree ended early because we returned.\n");
	...
	//printf("Final result from execution of tree '%s' is %s\n", name_.get(), rv.info());
	if (!result) printf("Execution FAILED.\n");
	return result;
}
...
// 	msg.enter("Tree::addOperator");
	...
	nodes_.own(leaf);
	//printf("Added operator '%s' (%p) to %p...\n", Command::data[func].keyword, leaf, this);
...
TreeNode *Tree::addArrayConstant(TreeNode *values)
{
	...
	if (baddata)
	{
		printf("Error: Incompatible mixture of data types found in array declaration.\n");
		return NULL;
	}
...
TreeNode *Tree::wrapVariable(Variable *var, TreeNode *arrayindex)
{
	...
	{
		printf("Error: Array index given to variable '%s', but it is not an array.\n", var->name());
		return NULL;
	}
	VariableNode *vnode = new VariableNode(var);
	nodes_.own(vnode);
...
// 	msg.enter("Tree::finalisePath");
	...
	if (ri == NULL)
	{
		printf("Internal Error: No path on stack to finalise.\n");
		return NULL;
	}
