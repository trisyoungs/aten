...
StepNode* AtomVariable::accessorSearch(QString name, TreeNode* arrayIndex, TreeNode* argList)
{
	...
	if (i == -1)
	{
		...
		{
			Messenger::print("Error: Syntax for 'Atom&' function '%s' is '%s(%s)'.", qPrintable(functionData[i].name), qPrintable(functionData[i].name), qPrintable(functionData[i].argText) );
			delete result;
			result = NULL;
		}
	}
	...
	{
		Messenger::print(Messenger::Parse, "Accessor match = %i (%s)", i, qPrintable(qPrintable(accessorData[i].name)));
		// Were we given an array index when we didn't want one?
		if ((accessorData[i].arraySize == 0) && (arrayIndex != NULL))
		{
			Messenger::print("Error: Irrelevant array index provided for member '%s'.", qPrintable(qPrintable(accessorData[i].name)));
			result = NULL;
		}
...
bool AtomVariable::retrieveAccessor(int i, ReturnValue& rv, bool hasArrayIndex, int arrayIndex)
{
	...
	{
		Messenger::print("Error: Unnecessary array index provided for member '%s'.", qPrintable(qPrintable(accessorData[i].name)));
		Messenger::exit("AtomVariable::retrieveAccessor");
		return false;
	}
	else if ((accessorData[i].arraySize > 0) && (hasArrayIndex))
	{
		...
		{
			Messenger::print("Error: Array index out of bounds for member '%s' (%i, range is 1-%i).", qPrintable(qPrintable(accessorData[i].name)), arrayIndex, accessorData[i].arraySize);
			Messenger::exit("AtomVariable::retrieveAccessor");
			return false;
		}
	}
