...
StepNode* GridVariable::accessorSearch(QString name, TreeNode* arrayIndex, TreeNode* argList)
{
	...
	if (i == -1)
	{
		...
		{
			Messenger::print("Error: Syntax for 'Grid&' function '%s' is '%s(%s)'.", qPrintable(functionData[i].name), qPrintable(functionData[i].name), qPrintable(functionData[i].argText) );
			delete result;
			result = NULL;
		}
	}
