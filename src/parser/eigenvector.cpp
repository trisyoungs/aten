...
StepNode* EigenvectorVariable::accessorSearch(QString name, TreeNode* arrayIndex, TreeNode* argList)
{
	...
	if (i == -1)
	{
		...
		{
			Messenger::print("Error: Syntax for 'EigenVector&' function '%s' is '%s(%s)'.", functionData[i].name, functionData[i].name, functionData[i].argText );
			delete result;
			result = NULL;
		}
	}
...
void EigenvectorVariable::printAccessors()
{
	if (EigenvectorVariable::nAccessors > 0)
	{
		...
		QString accessors;
		for (int n=0; n<EigenvectorVariable::nAccessors; ++n) accessors += QString("%1%2%3").arg(n == 0 ? " " : ", ", accessorData[n].name, accessorData[n].arraySize > 0 ? "[]" : "");
	...
	if ((EigenvectorVariable::nFunctions > 0) && (strcmp(functionData[0].name,".dummy") != 0))
	{
		...
		QString functions;
		for (int n=0; n<EigenvectorVariable::nFunctions; ++n) functions += QString("%1%2(%3)").arg(n == 0 ? " " : ", ", functionData[n].name, functionData[n].argText);
