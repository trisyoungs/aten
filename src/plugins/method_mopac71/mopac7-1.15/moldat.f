...
C$DOIT ASIS
   ...
   80 CONTINUE
      IF(USS(1) .GT. -1.D0) THEN
         WRITE(WU,'(''  THE HAMILTONIAN REQUESTED IS NOT AVAILABLE IN''
     1,'' THIS PROGRAM'')')
         MOLDAT=.FALSE.
         RETURN
      ENDIF
  ...
  130 IA=IB+1
      IF(NUMAT.EQ.1)THEN
         IF(INDEX(KEYWRD,'FORCE').NE.0)THEN
            WRITE(WU,'(///,A)')'      A SINGLE ATOM HAS NO VIBRATIONAL MO
     1DES'
            MOLDAT=.FALSE.
            RETURN
         ENDIF
      ...
      IF(NORBS.GT.MAXORB)THEN
         WRITE(WU,'(//10X,''**** MAX. NUMBER OF ORBITALS:'',I4,/
     ...
     2MAXORB,NORBS
      ...
      IF(N2EL.GT.N2ELEC)THEN
        WRITE(WU,'(//10X,''**** MAX. NUMBER OF TWO-ELECTRON INTEGRALS:''
     1,I8,/
...
C   NOW TO CALCULATE THE NUMBER OF LEVELS OCCUPIED
      ...
      IF(INDEX(KEYWRD,'C.I.') .NE. 0 .AND. UHF ) THEN
         WRITE(WU,'(//10X,''C.I. NOT ALLOWED WITH UHF '')')
         MOLDAT=.FALSE.
         RETURN
      ENDIF
C
      ...
      IF( UHF ) THEN
         ...
         IF( TRIP ) THEN
            IF(NBETA*2 .NE. NELECS) THEN
               WRITE(WU,'(//10X,''TRIPLET SPECIFIED WITH ODD NUMBER'',
     1            '' OF ELECTRONS, CORRECT FAULT '')')
               MOLDAT=.FALSE.
               RETURN
            ELSE
               IF(MODE.NE.1)
     1WRITE(WU,'(//'' TRIPLET STATE CALCULATION'')')
               NBETA=NBETA-1
            ENDIF
         ENDIF
         IF(INDEX(KEYWRD,'QUAR').NE.0) THEN
            IF(NBETA*2 .EQ. NELECS) THEN
               WRITE(WU,'(//10X,''QUARTET SPECIFIED WITH EVEN NUMBER'',
     1            '' OF ELECTRONS, CORRECT FAULT '')')
               MOLDAT=.FALSE.
               RETURN
            ELSE
               IF(MODE.NE.1)
     1WRITE(WU,'(//'' QUARTET STATE CALCULATION'')')
               NBETA=NBETA-1
            ENDIF
         ENDIF
         IF(INDEX(KEYWRD,'QUIN').NE.0) THEN
            IF(NBETA*2 .NE. NELECS) THEN
               WRITE(WU,'(//10X,''QUINTET SPECIFIED WITH ODD NUMBER'',
     1            '' OF ELECTRONS, CORRECT FAULT '')')
               MOLDAT=.FALSE.
               RETURN
            ELSE
               IF(MODE.NE.1)
     1WRITE(WU,'(//'' QUINTET STATE CALCULATION'')')
               NBETA=NBETA-2
            ENDIF
         ENDIF
         IF(INDEX(KEYWRD,'SEXT').NE.0) THEN
            IF(NBETA*2 .EQ. NELECS) THEN
               WRITE(WU,'(//10X,''SEXTET SPECIFIED WITH EVEN NUMBER'',
     1            '' OF ELECTRONS, CORRECT FAULT '')')
               MOLDAT=.FALSE.
               RETURN
            ELSE
              IF(MODE.NE.1)WRITE(WU,'(//'' SEXTET STATE CALCULATION'')')
              NBETA=NBETA-2
            ENDIF
         ENDIF
         NALPHA=NELECS-NBETA
         IF(MODE.NE.1)
     1WRITE(WU,'(//10X,''UHF CALCULATION, NO. OF ALPHA ELECTRONS ='',I
...
C
         ...
         IF( TRIP .OR. EXCI .OR. BIRAD ) THEN
            IF( (NELECS/2)*2 .NE. NELECS) THEN
               WRITE(WU,'(//10X,''SYSTEM SPECIFIED WITH ODD NUMBER'',
     1            '' OF ELECTRONS, CORRECT FAULT '')')
               MOLDAT=.FALSE.
               RETURN
            ENDIF
            IF(MODE.NE.1) THEN
               IF(BIRAD)WRITE(WU,'(//'' SYSTEM IS A BIRADICAL'')')
               IF(TRIP )WRITE(WU,'(//'' TRIPLET STATE CALCULATION'')')
               IF(EXCI )WRITE(WU,'(//'' EXCITED STATE CALCULATION'')')
            ENDIF
            IELEC=2
            ILEVEL=2
         ...
         ENDIF
         IF(INDEX(KEYWRD,'QUAR').NE.0) THEN
            IF(MODE.NE.1)WRITE(WU,'(//'' QUARTET STATE CALCULATION'')')
            IELEC=3
            ILEVEL=3
         ENDIF
         IF(INDEX(KEYWRD,'QUIN').NE.0) THEN
            IF(MODE.NE.1)WRITE(WU,'(//'' QUINTET STATE CALCULATION'')')
            IELEC=4
            ILEVEL=4
         ENDIF
         IF(INDEX(KEYWRD,'SEXT').NE.0) THEN
     ...
     1                  (IELEC/2)*2.EQ.IELEC) THEN
               WRITE(WU,'('' IMPOSSIBLE NUMBER OF OPEN SHELL ELECTR
     1ONS'')')
               MOLDAT=.FALSE.
               RETURN
            ENDIF
            ...
            IF(NCLOSE+NOPEN.GT.NORBS)THEN
               WRITE(WU,'(A)')' NUMBER OF DOUBLY FILLED PLUS PARTLY FILLE
     1D LEVELS'
               WRITE(WU,'(A)')' GREATER THAN TOTAL NUMBER OF ORBITALS.'
               MOLDAT=.FALSE.
               RETURN
            ENDIF
            FRACT=IELEC*1.D0/ILEVEL
            IF(MODE.NE.1)
     1WRITE(WU,'('' THERE ARE'',I3,'' DOUBLY FILLED LEVELS'')')NCLOSE
         ENDIF
         IF(MODE.NE.1)WRITE(WU,'(//10X,''RHF CALCULATION, NO. OF '',
     1''DOUBLY OCCUPIED LEVELS ='',I3)')NCLOSE
         IF(MODE.NE.1 .AND. NOPEN.NE.0.AND.ABS(FRACT-1.D0).LT.1.D-4)
     1WRITE(WU,'(/27X,''NO. OF SINGLY OCCUPIED LEVELS ='',I3)')NOPEN
         IF(MODE.NE.1 .AND. NOPEN.NE.0.AND.ABS(FRACT-1.D0).GT.1.D-4)
     1WRITE(WU,'(/27X,''NO. OF LEVELS WITH OCCUPANCY'',F6.3,''  ='',I3)'
     2)FRACT,NOPEN
         IF(INDEX(KEYWRD,'C.I.=(').NE.0) THEN
            I=READA(KEYWRD,INDEX(KEYWRD,'C.I.=(')+5)-
     1      READA(KEYWRD,INDEX(KEYWRD,'C.I.=(')+7)
            IF(NOPEN.GT.I)THEN
               WRITE(WU,'(//,'' NUMBER OF OPEN-SHELLS ALLOWED IN C.I. IS
...
C
     ...
     1' SPECIFIED SPIN COMPONENT NOT SPANNED BY ACTIVE SPACE'
         ...
         ENDIF
      ENDIF
C#      WRITE(WU,'(''  NOPEN,NCLOSE,NALPHA,NBETA,FRACT'',4I4,F12.5)')
...
C
     ...
     1INDEX(KEYWRD,'ANALYT').NE.0.AND.HALFE)THEN
         WRITE(WU,*)
         WRITE(WU,'(A)')' KEYWORD ''ANALYT'' CANNOT BE USED HERE: ',
  ...
  220 CONTINUE
      IF(MODE.NE.1.AND.NNHCO.NE.0)THEN
         IF(INDEX(KEYWRD,'MMOK').NE.0) THEN
            WRITE(WU,'(A)')' MOLECULAR MECHANICS CORRECTION APPLIED TO PE
     1PTIDELINKAGE'
         ELSEIF(INDEX(KEYWRD,'NOMM').NE.0)THEN
            WRITE(WU,'(A,I2,2A)')' THERE ARE ',NNHCO/2,' PEPTIDE LINKAGES
     1',' IDENTIFIED IN THIS SYSTEM'
            WRITE(WU,'(A)')' IF YOU WANT MM CORRECTION TO THE CONH BARRIE
     1R, ADD THE KEY-WORD "MMOK"'
            NNHCO=0
         ELSE
            WRITE(WU,'(A)')' THIS SYSTEM CONTAINS -HNCO- GROUPS.'
            WRITE(WU,'(A)')' YOU MUST SPECIFY "NOMM" OR "MMOK" REGARDING
