...
C
  100 NPNTS=NPNTS+1
      IF(DEBUG)WRITE(WU,'(''   INTERPOLATED ENERGY:'',F13.6)')E*23.061D0
      IPOINT=NPNTS
      GO TO 500
C
...
C
      ...
      DO 550 K=1,MINPQ
         CK=COS(XNOW*THETA(K))
         SK=SIN(XNOW*THETA(K))
         IF(DEBUG)WRITE(WU,'('' ROTATION ANGLE:'',F12.4)')SK*57.29578D0
         DO 540 I=1,N
...
C
  ...
  580 ROLD=XOLD*THETA(1)*57.29578D0
      ...
      IF(DEBUG)WRITE(WU,600) XOLD,EOLD*23.061D0,DEOLD,ROLD
     ...
     2,             XMIN,EMIN*23.061D0,DEMIN,RMIN
      ...
      WRITE(WU,610)
      DO 590 K=1,NPNTS
  590 WRITE(WU,620) K,X(K),F(K),DF(K)
      WRITE(WU,630)
      RETURN
  600 FORMAT(
...
************************************************************************
      ...
      IF(START) THEN
         ...
         IF(DEBUG)
     1    WRITE(WU,'('' MAXIMUM SIZE:'',I5)')MFOCK
         NFOCK=1
         LFOCK=1
         START=.FALSE.
      ELSE
...
*
   ...
   30 EMAT(I,J)=EMAT(I,J)*CONST
      IF(DEBUG) THEN
         WRITE(WU,'('' EMAT'')')
         DO 40 I=1,NFOCK1
   40    WRITE(WU,'(6E13.6)')(EMAT(J,I),J=1,NFOCK1)
      ENDIF
      L=0
      DO 50 I=1,NFOCK1
         DO 50 J=1,NFOCK1
...
*********************************************************************
   ...
   70 COEFFS(I)=-EVEC(I+IL)
      IF(DEBUG) THEN
         WRITE(WU,'('' EVEC'')')
         WRITE(WU,'(6F12.6)')(COEFFS(I),I=1,NFOCK)
         WRITE(WU,'(''    LAGRANGIAN MULTIPLIER (ERROR) =''
