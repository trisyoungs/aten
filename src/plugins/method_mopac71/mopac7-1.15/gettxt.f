...
C CONVERTED 07/2016 BY TGAY FROM SUBROUTINE TO LOGICAL FUNCTION
      ...
      LOGICAL FUNCTION GETTXT(DATAFILE)
...
c *******
      ...
      write(0,'(A80)') DATAFILE
      IREC=IREC+1
      IF (IREC.GT.NUMREC) GOTO 90
      READ(DATAFILE(IREC:IREC),'(A)',END=100,ERR=90)KEYWRD(:80)
      write(0,*) "KEYWORD=", KEYWRD
      OLDKEY=KEYWRD
   ...
   10    CONTINUE
         READ(DATAFILE,'(A)',END=100,ERR=90)KOMENT,TITLE
...
C
         ...
         IREC=IREC+1
         IF (IREC.GT.NUMREC) GOTO 90
         READ(DATAFILE(IREC:IREC),'(A)',END=100,ERR=90)KEYWRD(81:160)
...
C
        write(0,*) "THIRDKEYWD"
            IREC=IREC+1
            IF (IREC.GT.NUMREC) GOTO 90
            READ(DATAFILE(IREC:IREC),'(A)',END=100,ERR=90)
     1KEYWRD(161:240)
            CALL UPCASE(KEYWRD(161:240))
         ENDIF
C
...
C
         ...
         IREC=IREC+1
         IF (IREC.GT.NUMREC) GOTO 90
         READ(DATAFILE(IREC:IREC),'(A)',END=100,ERR=90)KOMENT
         IREC=IREC+1
         IF (IREC.GT.NUMREC) GOTO 90
         READ(DATAFILE(IREC:IREC),'(A)',END=100,ERR=90)TITLE
...
C               stop
   ...
   30       CONTINUE
     ...
     1KEYWRD(161:240)
      ...
      ENDIF
      GOTO 50
   40 WRITE(WU,'(A)')' SETUP FILE MISSING OR CORRUPT'
   50 DO 80 J=1,3
        write(0,*) "J = ", J
         IF(KEYWRD(IS(J):IS(J)) .NE. ' ') THEN
            ...
            DO 60 I=IS(J)+1,239
               ...
               ENDIF
   60       CONTINUE
            WRITE(WU,'(A,I2,A)')' LINE',J,' OF KEYWORDS DOES NOT HAVE 
     1ENOUGH'
            WRITE(WU,'(A)')' SPACES FOR PARSING.  PLEASE CORRECT LINE.'
            GETTXT=.FALSE.
            RETURN
   70       CONTINUE
         ENDIF
   80 CONTINUE
      RETURN
   90 WRITE(WU,'(A)')' ERROR IN READ OF FIRST THREE LINES'
  100 GETTXT=.FALSE.
      END
