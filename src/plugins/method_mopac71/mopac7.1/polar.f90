...
!-----------------------------------------------
      ...
      use permanent_arrays, only : nat, na, labels, &
      & geo, coord, w, c
...
!-----------------------------------------------
      real(double)  :: x1(norbs,norbs) 
      real(double)  :: x2(norbs,norbs) 
      real(double)  :: x3(norbs,norbs) 
      real(double)  :: x4(norbs,norbs) 
      real(double)  :: x5(norbs,norbs) 
      real(double)  :: x6(norbs,norbs) 
      real(double)  :: x7(norbs,norbs) 
      real(double)  :: x8(norbs,norbs) 
      real(double)  :: x19(norbs,norbs) 
      real(double)  :: x10(norbs,norbs) 
      real(double)  :: x11(norbs,norbs) 
      real(double)  :: x12(norbs,norbs) 
      real(double)  :: x13(norbs,norbs) 
!-----------------------------------------------
...
!-----------------------------------------------
      integer :: mass, i, j, k, l, iwflb, ibet, igam, maxitu, maxita, &
        nfreq, iwfla 
      real(double), dimension(nvar) :: grad 
      real(double), dimension(3,3) :: rotvec, tempv 
      real(double), dimension(10) :: dataev 
      real(double), dimension(3,numat) :: refgeo 
      real(double) :: a, b, sumw, sum, sumx, sumy, sumz, summax, atpol, heat0, &
...
!..
      ...
      write (iw, 10) 
   10 format('1',20('*'),' TDHF POLARIZABILITIES ',20('*'),/,/) 
...
!
      if (.not.let) then 
        mass = 1 
        call axis (coord, numat, a, b, cc, sumw, mass, rotvec) 
        mass = Int(a + b + cc + sumw) ! dummy use of arguments of axis
        write (iw, 20) 
...
!   SECOND HARMONIC GENERATION
        if (ibet == (-1)) then 
          iwfla = 2 
          omega = omega*2.0D00 
          call alphaf (iwfla, atol, maxita, x1, x2, x3, x4, x5, x6, x7, x8, c, &
            w) 
          omega = omega/2.0D00 
...
!-----------------------------------------------
      ...
      use chanel_C, only : iw
      use permanent_arrays, only : eigs, nat, nfirst, nlast, coord
      USE polar_C, only : omega 
!
...
!   L o c a l   V a r i a b l e s
!-----------------------------------------------
      integer :: nsqr, iposu, iposg, id, icount, ic, j 
      real(double), dimension(3,3) :: allalp 
      real(double) :: alpavg, cmptim, alpold, diff, alphaw, dela 
      logical :: last, debug 
...
!
   ...
   50     format('      ALPHA(',a1,',',a1,') = ',1p,d14.7) 
      ...
      write (iw, '(10X,'' *=X:'',3F13.5)') (allalp(1,j),j=1,3) 
      write (iw, '(10X,'' *=Y:'',3F13.5)') (allalp(2,j),j=1,3) 
      write (iw, '(10X,'' *=Z:'',3F13.5)') (allalp(3,j),j=1,3) 
...
!-----------------------------------------------
      USE vast_kind_param, ONLY:  double 
      use molkst_C, only : norbs, numat, nclose
      use permanent_arrays, only : eigs, nat, nfirst, nlast, coord
      use chanel_C, only : iw
      USE polar_C, only : omega 
!
...
!
  ...
  100 format(/,/,' AVERAGE BETAX VALUE AT ',f10.5,' EV = ',1f15.5) 
      write (iw, 110) omega, bavy 
  110 format(' AVERAGE BETAY VALUE AT ',f10.5,' EV = ',1f15.5) 
      write (iw, 120) omega, bavz 
  120 format(' AVERAGE BETAZ VALUE AT ',f10.5,' EV = ',1f15.5) 
      write (iw, 130) omega, bvec 
  130 format(/,/,'  AVERAGE BETA VALUE AT ',f10.5,' EV = ',1f15.5,/,/) 
!
      return  
...
!-----------------------------------------------
      ...
      use permanent_arrays, only : eigs, nat, nfirst, nlast, coord
      use chanel_C, only : iw
      use molkst_C, only : norbs, nclose, numat, keywrd
      USE polar_C, only : omega 
!
...
!  EPSILON IN H1 AND UMINUS IN DA
      ...
      &Z'', 9X,''*ZZ'')') 
      write (iw, '( 3X,''*=X'',6F12.5)') ((allbet(1,i,j),i=1,j),j=1,3) 
      write (iw, '( 3X,''*=Y'',6F12.5)') ((allbet(2,i,j),i=1,j),j=1,3) 
      write (iw, '( 3X,''*=Z'',6F12.5)') ((allbet(3,i,j),i=1,j),j=1,3) 
!
      bavx = bavx/5.0D+00 
      bavy = bavy/5.0D+00 
      bavz = bavz/5.0D+00 
...
!
   ...
   90 format(/,/,' AVERAGE BETAX(SHG) VALUE AT',f10.5,' EV = ',1f11.5) 
      write (iw, 100) omega, bavy 
  100 format(' AVERAGE BETAY(SHG) VALUE AT',f10.5,' EV = ',1f11.5) 
      write (iw, 110) omega, bavz 
  110 format(' AVERAGE BETAZ(SHG) VALUE AT',f10.5,' EV = ',1f11.5) 
!
      write (iw, 120) omega, bvec 
  120 format(/,/,' AVERAGE BETA (SHG) VALUE AT',f10.5,' EV = ',1f11.5,/,/) 
      return  
...
!-----------------------------------------------
      USE vast_kind_param, ONLY:  double 
      USE molkst_C, only : lm6, numat, norbs
...
!   L o c a l   V a r i a b l e s
!-----------------------------------------------
      integer :: ii, iim1, ia, ib, jj, ja, jb, ijw, i, j, klw, k, l, kk 
...
!
      do k = nclose + 1, norbs 
        u(:nclose,k) = hartr*g(:nclose,k)/(eigs(k)-eigs(:nclose)-omega) 
        u(k,:nclose) = hartr*g(k,:nclose)/(eigs(:nclose)-eigs(k)-omega) 
      end do 
!
!  CHECK FOR CONVERGENCE
!
...
! WRITE GAMMA(ABCD)
!
        write (iw, 90) alab(ia), alab(ib), alab(ic), alab(id), gamma(ie) 
...
!
      ...
      gave = gav/15.0D+00 
      write (iw, 100) omega, gave 
  100 format(/,/,' AVERAGE GAMMA VALUE AT ',f10.5,' EV = ',1p,d14.7,/,/) 
      return  
...
!
      ...
      gave = gav/15.0D+00 
      write (iw, 90) omega, gave 
   90 format(/,/,'  AVERAGE GAMMA VALUE AT ',f10.5,' = ',1p,d14.7,/,/) 
      return  
...
!
      ...
      gave = gav/15.0D+00 
      write (iw, 100) omega, gave 
  100 format(/,/,'  AVERAGE GAMMA VALUE AT ',f10.5,' = ',1p,d14.7,/,/) 
      return  
...
!
      write (iw, 30) omega, bavx 
   30 format(/,/,' AVERAGE BETAX(SHG) VALUE AT',f10.5,'EV = ',1f15.5) 
!
      write (iw, 40) omega, bavy 
   40 format(' AVERAGE BETAY(SHG) VALUE AT',f10.5,'EV = ',1f15.5) 
!
      write (iw, 50) omega, bavz 
   50 format(' AVERAGE BETAZ(SHG) VALUE AT',f10.5,'EV = ',1f15.5,/,/) 
!
      write (iw, 60) omega, bvec 
   60 format(/,/,' AVERAGE BETA (SHG) VALUE AT',f10.5,'EV = ',1f15.5,/,/) 
      return  
...
!
   ...
   10 format(/,/,'  BXXX  ',d15.8,'  BYXX ',d15.8,'  BZXX ',d15.8,/,'  BXXY  ',&
        ...
        d15.8,/,'  BXZZ  ',d15.8,'  BYZZ ',d15.8,'  BZZZ ',d15.8) 
!
      write (iw, 20) omega, bavx 
   20 format(/,/,' AVERAGE BETAX VALUE AT',f10.5,'EV = ',1f15.5) 
!
      write (iw, 30) omega, bavy 
   30 format(' AVERAGE BETAY VALUE AT',f10.5,'EV = ',1f15.5) 
!
      write (iw, 40) omega, bavz 
   40 format(' AVERAGE BETAZ VALUE AT',f10.5,'EV = ',1f15.5,/,/) 
!
      write (iw, 50) omega, bvec 
   50 format(/,/,' AVERAGE BETA(EOPE) VALUE AT',f10.5,'EV = ',1f15.5,/,/) 
      return  
...
!
      write (iw, 30) omega, bavx 
   30 format(/,/,' AVERAGE BETAX VALUE AT ',f10.5,'EV = ',1f15.5) 
!
      write (iw, 40) omega, bavy 
   40 format(' AVERAGE BETAY VALUE AT ',f10.5,'EV = ',1f15.5) 
!
      write (iw, 50) omega, bavz 
   50 format(' AVERAGE BETAZ VALUE AT ',f10.5,'EV = ',1f15.5,/,/) 
!
      write (iw, 60) omega, bvec 
   60 format(/,/,' AVERAGE BETA(OR) VALUE AT ',f10.5,'EV = ',1f15.5,/,/) 
!
      return  
...
!
      i = index(jobnam,' ') - 1 
      open(unit=idaf, file=jobnam(:i)//'.pol', status='UNKNOWN', access=&
