      real(kind(0.0d0)) function dipole (p, q, coord, dipvec, mode) 
!-----------------------------------------------
!   M o d u l e s 
!-----------------------------------------------
      USE vast_kind_param, ONLY:  double  
      USE molkst_C, only : numat, keywrd, numcal, ux, uy, uz, mpack
      USE parameters_C, only : ddp
      USE funcon_C, only : fpc_8, fpc_1, a0
      USE chanel_C, only : iw
      use permanent_arrays, only : nfirst, nlast, nat, atmass
...
!-----------------------------------------------
      ...
      real(double) :: wtmol, sum, hyfsp, xt, hyfpd, dx, dy, dz, factdm 
      logical :: first, chargd, force 

      save dip, first, wtmol, chargd, force, icalcn 
!-----------------------------------------------
!***********************************************************************
...
!
      if (first) then 
        ...
        sum = 0.D0 
        do i = 1, numat 
          wtmol = wtmol + atmass(i) 
          sum = sum + q(i) 
        end do 
        chargd = abs(sum) > 0.5D0 
        force = index(keywrd,'FORCE') + index(keywrd,'IRC') /= 0 
      endif 
...
!
      ...
      do i = 1, numat 
        ...
        l = ib - ia 
        if (l > 0) then  
            hyfsp = 2.0D0*ddp(2,ni)*a0*fpc_8*fpc_1*1.D-10 
          do j = 1, 3 
            k = ((ia + j)*(ia + j - 1))/2 + ia 
