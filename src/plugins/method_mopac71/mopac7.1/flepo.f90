...
!...Translated by Pacific-Sierra Research 77to90  4.4G  15:29:41  03/15/06  
...
!-----------------------------------------------
!-----------------------------------------------
      call flepn (xparam, nvar, funct1) 
      return  
      end subroutine flepo 


      subroutine flepn(xparam, nvar, funct1) 
!-----------------------------------------------
!   M o d u l e s 
!-----------------------------------------------
      USE vast_kind_param, ONLY:  double 
      use molkst_C, only : numcal, gnorm, iflepo, keywrd, emin, tleft, &
      & time0, moperr, nscf, limscf, tdump, last, cosine
      use permanent_arrays, only : hesinv, grad
...
!
        ...
        write (iw, '('' GRADIENT CRITERION IN FLEPO ='',F12.5)') tolerg 
      endif 
      if (nvar == 1) then 
        pvect(1) = 0.01D0 
        alpha = 1.D0 
...
!
  ...
  270 continue 
      ...
      if (nvar == 1) then 
        write (iw, &
      '('' ONLY ONE VARIABLE, '',                           ''THEREFORE ENERGY &
      &A MINIMUM'')') 
        last = 1 
        lgrad = index(keywrd,'GRAD') /= 0 
        grad(1) = 0.D0 
...
!
  ...
  300   format(' ',20x,'COS WILL BE RESET AND ANOTHER ','ATTEMPT MADE') 
      ...
      if (absmin - smval < 1.D-7) then 
        ...
        if (itry1 > 10) then 
          write (iw, &
      '(2/,'' HEAT OF FORMATION IS '',                   ''ESSENTIALLY STATIONA&
      &RY'')') 
          go to 420 
        endif 
      else 
...
!
      ...
      if (resfil) then 
        if (minprt) write (iw, 440) tprt, txt, min(gnorm,999999.999D0), funct1 
        if (log) write (ilog, 450) jcyc, min(tcycle,9999.99D0), tprt, txt, min(&
          gnorm,999999.999D0), funct1 
  440   format(' RESTART FILE WRITTEN,    TIME LEFT:',f6.2,a1,'  GRAD.:',f10.3,&
          ' HEAT:',g13.7) 
        resfil = .FALSE. 
      else 
        if (minprt) write (iw, 450) jcyc, min(tcycle,9999.99D0), tprt, txt, &
          min(gnorm,999999.999D0), funct1 
        if (log) write (ilog, 450) jcyc, min(tcycle,9999.99D0), tprt, txt, min(&
          gnorm,999999.999D0), funct1 
  450   format(' CYCLE:',i4,' TIME:',f8.3,' TIME LEFT:',f6.2,a1,'  GRAD.:',&
          f10.3,' HEAT:',g13.7) 
      endif 
      if (minprt) then 
        endfile (iw) 
  ...
  460 format(20x,'THERE IS NOT ENOUGH TIME FOR ANOTHER CYCLE',/,30x,&
      ...
      iflepo = -1 
      return  
      end subroutine flepn 

      subroutine setup_flepo(n)
      use permanent_arrays, only : hesinv
