...
!-----------------------------------------------
!   I norbs t e r f a c e   B l o c k s
!-----------------------------------------------
      use rsp_I, only :
      use schmit_I, only :
      use schmib_I 
      ...
      implicit none
!-----------------------------------------------
!   D u m m y   A r g u m e norbs t s
!-----------------------------------------------
      integer  :: np 
      integer  :: nq 
...
!-----------------------------------------------
      integer ::  icalcn, minpq, np1, np2, ii, i, i1, j, k, ik, ipoint, j1, &
        ij, il, k1, k2 
      real(double), dimension(norbs) :: theta 
      real(double) :: eold, xold, zero, ff, dum, dedx, deold, xnow, denow, enow&
        , ck, sk
      real(double)  :: vec(norbs,norbs)
      real(double)  :: fock(norbs,norbs) 
      real(double)  :: p(norbs,norbs) 
      real(double)  :: h(norbs*norbs) 
      real(double)  :: vecl(norbs*norbs)
      integer :: npnts 
      real(double), dimension(12) :: x, f, df 
      real(double) :: xlow, xhigh, xmin, emin
      logical :: debug 

      save debug, eold, xold, zero, ff, icalcn 
!-----------------------------------------------
!*********************************************************************
!
! INTERP: AN INTERPOLATION PROCEDURE FOR FORCING SCF CONVERGANCE
!         ORIGINAL THEORY AND FORTRAN WRITTEN BY R.norbs. CAMP AND
...
!                  AS PACKED ARRAY OF norbs*norbs COEFFICIENTS
!
...
!
        ...
        do i = 1, np 
          do j = 1, nq 
            dum = zero 
            dum = dum + sum(cp(:norbs,i)*p(:norbs,j)) 
            fock(i,j) = dum 
          end do 
        end do 
        if (mode /= 3) then 
...
!
          vec(:norbs,:norbs) = cp(:norbs,:norbs) 
          eold = e 
          xold = 1.0D0 
          mode = 2 
          return  
...
!
        ...
        do i = 1, norbs 
          ...
          do j = j1, norbs 
            p(i,j) = zero 
            p(i,j) = p(i,j) + sum(cp(:norbs,i)*vec(:norbs,j)) 
          end do 
        end do 
!
...
!
        ...
        do i = 1, np 
          do j = 1, i 
            ij = ij + 1 
            h(ij) = 0.D0 
            h(ij) = h(ij) + sum(p(i,np1:norbs)*p(j,np1:norbs)) 
          end do 
        end do 
        call rsp (h, np, np, theta, vecl) 
        do i = np, 1, -1 
          il = i*np - np 
          vec(np:1:(-1),i) = vecl(np+il:il+1:(-1)) 
        end do 
        do i = 1, np2 
          dum = theta(np1-i) 
          theta(np1-i) = theta(i) 
...
!
        do i = 1, nq 
          do j = 1, minpq 
            vec(i,np+j) = zero 
            vec(i,np+j) = vec(i,np+j) + sum(p(:np,np+i)*vec(:np,j)) 
          end do 
        end do 
        call schmit (vec(1,np1), nq, norbs) 
!
...
!
        do i = 1, np 
          ...
          do j = 1, np 
            p(i,j) = zero 
            p(i,j) = p(i,j) + sum(h(:np)*vec(:np,j)) 
          end do 
        end do 
        call schmib (p, np, norbs) 
!
!     CALCULATE VQ MATRIX AND HOLD IN LAST NQ COLUMNS OF P MATRIX.
!
        do i = 1, nq 
          h(:nq) = p(np+i,np+1:nq+np) 
          do j = np1, norbs 
            p(i,j) = zero 
            p(i,j) = p(i,j) + sum(h(:nq)*vec(:nq,j)) 
          end do 
        end do 
        call schmib (p(1,np1), nq, norbs) 
!
...
!
        ...
        do i = 1, np 
          do j = 1, nq 
            dum = zero 
            dum = dum + sum(theta(:minpq)*p(i,:minpq)*vec(j,np+1:minpq+np)) 
...
!
        ...
        do j = 1, norbs 
          ...
          do i = 1, norbs 
            dum = zero 
            dum = dum + sum(cp(i,k1+1:k2+k1)*p(:k2,j)) 
            vec(i,j) = dum 
          end do 
        end do 
!
...
!
        ...
        do i = 1, norbs 
          ...
          do j = 1, nq 
            dum = zero 
            dum = dum + sum(fp(ii+1:i+ii)*vec(:i,j+np)) 
            if (i /= norbs) then 
              ik = ii + i + i 
              do k = i1, norbs 
        ...
        do i = 1, np 
          do j = 1, nq 
            dum = zero 
            dum = dum + sum(vec(:norbs,i)*p(:norbs,j)) 
            fock(i,j) = dum 
          end do 
        end do 
!
...
!
      ...
      if (eold - enow<=ff*(eold - emin) .and. ipoint<=10) then 
!
...
!
        ...
        do k = 1, minpq 
          ...
          cp(:norbs,k) = ck*vec(:norbs,k) - sk*vec(:norbs,np+k) 
          cp(:norbs,np+k) = sk*vec(:norbs,k) + ck*vec(:norbs,np+k) 
        end do 
        mode = 3 
        return  
      endif 
