...
C
         WRITE(WU,'(A)')' INITIAL VELOCITY IN DRC'
         WRITE(WU,'(3F13.5)')(STARTV(I),I=1,NUMAT*3)
      ENDIF
      LET=(INDEX(KEYWRD,' GEO-OK').NE.0.OR.VELRED)
      IF(INDEX(KEYWRD,' SYM').NE.0)THEN
         WRITE(WU,*)'  SYMMETRY SPECIFIED, BUT CANNOT BE USED IN DRC'
         NDEP=0
      ENDIF
C
...
C
      IF(INDEX(KEYWRD,'DRC=').NE.0) THEN
        ...
        WRITE(WU,'(//10X,'' DAMPING FACTOR FOR KINETIC ENERGY ='',F12.6)
     1')HALF
...
C
      ...
      IF(INDEX(KEYWRD,'KINE').NE.0) THEN
        ...
        WRITE(WU,'(//10X,'' EXCESS KINETIC ENERGY ENTERED INTO SYSTEM ='
     1',F12.6)')ADDONK
      ELSE
         ADDONK=0.D0
      ENDIF
...
!     +FORM='UNFORMATTED')
     ...
     1GNORM,LETOT,ELOST1,GTOT
         WRITE(WU,'(//10X,''CALCULATION RESTARTED, CURRENT'',
   ...
   80       SUMM=SUMM+VELO0(I)**2*AMS
            ...
            IF(ADDONK.LT.1.D-5.AND..NOT.VELRED)THEN
               IF(ABS(HALF).GT.1.D-3.AND.STARTK(K).GT.105.D0)THEN
                  WRITE(WU,'(A,F10.3,A,/,A)')' BY DEFAULT, ONE QUANTUM OF
...
C
                  ...
                  WRITE(WU,'(A,F7.2,A)')' THIS REPRESENTS AN ENERGY OF',A
     1DDONK,' KCALS/MOLE'
               ELSEIF(ABS(HALF).GT.1.D-3)THEN
                  WRITE(WU,'(A,F9.2,A)')' THE VIBRATIONAL FREQUENCY (',ST
...
C
            IF(SUMM.LT.1.D-4) THEN
               WRITE(WU,'(A)')' SYSTEM IS APPARENTLY NOT MOVING!'
               RETURN
            ENDIF
C
...
C
  ...
  170    SUM=SUM + ((GRAD(I)-GROLD(I))/4.184D18)**2
     ...
     1      ILOOP.GT.30.AND.SCFOLD-ESCF.LT.0.D0)  THEN
               WRITE(WU,'(//,'' IRC CALCULATION COMPLETE '')')
               RETURN
            ENDIF
         ELSE
  ...
  180       PAST10(I)=PAST10(I+1)
            IF(SUM.LT.GNLIM)THEN
              WRITE(WU,'(//,A)')' GRADIENT CONSTANT AND SMALL -- ASSUME'
...
!     +FORM='UNFORMATTED')
     ...
     1EKIN,ELOST,ETOT,VELO0,NVAR)
            ...
            WRITE(WU,'(//10X,'' RUNNING OUT OF TIME, RESTART FILE WRITTEN
     1'')')
            WRITE(WU,'(A)')' GEOMETRY AND VELOCITY ARE IN RESTART FILE'
     1//' IN ASCII'
            RETURN
         ENDIF
  190 CONTINUE
