      LOGICAL FUNCTION GETGEO(DATAFILE,LABELS,GEO,LOPT,NA,NB,NC,AMS,
     1NATOMS,INT)
...
*
* MODIFIED 07/2016 BY TGAY TO READ FROM INTERNAL FILE SOURCE
* CONVERTED 07/2016 BY TGAY FROM SUBROUTINE TO LOGICAL FUNCTION
*
...
*  ATOMIC NUMBER USED: NO ISOTOPE ALLOWED
         ...
         IF (LABEL.LT.0.OR.LABEL.GT.107) THEN
            WRITE(WU,'(''  ILLEGAL ATOMIC NUMBER'')')
            GO TO 240
         END IF
         GO TO 80
      END IF
...
*   CHECK FOR ERROR IN ATOMIC SYMBOL
   ...
   70 CONTINUE
      IF(ELE(1:1).EQ.'X')THEN
         ...
         GOTO 80
      ENDIF
      WRITE(WU,'(''  UNRECOGNIZED ELEMENT NAME: ('',A,'')'')')ELE
      GOTO 240
*
* ALL O.K.
*
   80 IF (LABEL.NE.99) NUMAT=NUMAT+1
     ...
     1NUMATM
      ...
      IF(IRCDRC)THEN
         ...
         IF(TURN.EQ.'T')THEN
            LOPT(1,NATOMS)=1
            IF(NATOMS.EQ.1)WRITE(WU,'(A)')' IN DRC MONITOR POTENTIAL ENER
     1GY'//' TURNING POINTS'
         ELSE
            LOPT(1,NATOMS)=0
         ENDIF
...
C
                  WRITE(WU,'(A)')' GEOMETRY IS FAULTY.  GEOMETRY READ IN
     1IS'
...
*
  ...
  130 NA(2)=1
     ...
     1WRITE(WU,'(A)') ' WARNING: INTERNAL COORDINATES ARE ASSUMED 
     2FOR THREE-ATOM SYSTEMS'
         INT=.TRUE.
      ENDIF
      IF(INT)GEO(2,2)=0
C
...
C#      WRITE(WU,'(/10X,A)')'INITIAL VELOCITY VECTOR FOR DRC'
         DO 150 I=1,NATOMS
            IREC=IREC+1
            IF (IREC.GT.NUMREC) THEN
              WRITE(WU,'(A)') "Exhausted internal file data reading vel"
              GETGEO=.FALSE.
              RETURN
            ENDIF
            READ(DATAFILE(IREC:IREC),'(A)')LINE
...
C#      LABELS(NATOMS+1)=1
C#      LABELS(NATOMS+2)=1
C#      WRITE(WU,*)' FIRST ROTATION, ABOUT 1-2 X-AXIS'
         DO 170 I=1,NATOMS+2
            TEMP1= REACT(2,I)*CA+REACT(3,I)*SA
            TEMP2=-REACT(2,I)*SA+REACT(3,I)*CA
            REACT(2,I)=TEMP1
            REACT(3,I)=TEMP2
C#      WRITE(WU,'(2X,A2,2X,3F13.5)')ELEMNT(LABELS(I)),(REACT(J,I),J=1,3)
  170    CONTINUE
...
C#      WRITE(WU,*)' SECOND ROTATION, ABOUT 1-2 Z-AXIS'
         DO 180 I=1,NATOMS+2
...
C#      WRITE(WU,*)' THIRD ROTATION, ABOUT 2-3 X-AXIS'
         DO 190 I=1,NATOMS+2
            TEMP1= REACT(2,I)*CA+REACT(3,I)*SA
            TEMP2=-REACT(2,I)*SA+REACT(3,I)*CA
            REACT(2,I)=TEMP1
...
C
  ...
  220       LOPT(J,I)=0
     ...
     1THEN
            WRITE(WU,'(A)')' DUE TO PROGRAM BUG, THE FIRST THREE ATOMS MU
     ...
     1        LOPT(3,3) .GT. 0)THEN
            ...
            LOPT(3,2)=0
            LOPT(3,3)=0
           WRITE(WU,'(//10X,'' AN UNOPTIMIZABLE GEOMETRIC PARAMETER HAS'
...
* ERROR CONDITIONS
  230 IF(IREAD.EQ.5) THEN
         WRITE(WU,'( '' ERROR DURING READ AT ATOM NUMBER '', I3 )')NATOMS
      ELSE
         NATOMS=0
         RETURN
      ENDIF
  240 J=NATOMS-1
      WRITE(WU,'('' DATA CURRENTLY READ IN ARE '')')
      DO 250 K=1,J
  250 WRITE(WU,260)LABELS(K),(GEO(JJ,K),LOPT(JJ,K),JJ=1,3),
     1NA(K),NB(K),NC(K)
