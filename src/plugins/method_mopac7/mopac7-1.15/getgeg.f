...
C CONVERTED 07/2016 BY TGAY FROM SUBROUTINE TO LOGICAL FUNCTION
      ...
      LOGICAL FUNCTION GETGEG(DATAFILE,LABELS,GEO,NA,NB,NC,AMS,NATOMS,
     1INT)
...
c *******
      ...
      DO 60 NATOMS=1,100
         IREC=IREC+1
         IF (IREC.GT.NUMREC) GOTO 70
         READ(DATAFILE(IREC:IREC),'(A)',END=70,ERR=70)LINE
         IF(LINE.EQ.' ')GOTO 70
C
   ...
   30    IF(INDEX(' '//LINE(ISTART(1):ISTART(1)+2),ELEMNT(J)//' ').NE.0)
     1 GOTO 40
         IF(INDEX(' '//LINE(ISTART(1):ISTART(1)+2),' X').NE.0) THEN
            ...
            GOTO 40
         ENDIF
         WRITE(WU,'(2A)')' ELEMENT NOT RECOGNIZED: ',
   ...
   40    LABELS(NATOMS)=J
         IF(J.NE.99)THEN
            ...
            IF(ATMASS(NUMAT).GT.1.D-15)THEN
               WRITE(WU,'('' FOR ATOM'',I4,''  ISOTOPIC MASS:''
     1    ,F15.5)')NATOMS, ATMASS(NUMAT)
            ELSE
               ATMASS(NUMAT)=AMS(J)
            ENDIF
C#         WRITE(WU,*)NATOMS,NUMAT,ATMASS(NUMAT)
         ENDIF
         TGEO(1,NATOMS)=' '
         TGEO(2,NATOMS)=' '
         TGEO(3,NATOMS)=' '
   ...
   80 LGEO(J,I)=-1
      ...
      KERR=0
   90 IREC=IREC+1
      IF (IREC.GT.NUMREC) GOTO 180
      READ(DATAFILE(IREC:IREC),'(A)',END=180,ERR=180)LINE
      IF(LINE.EQ.' ')THEN
         IF(IVAR.EQ.-1)THEN
            MERR=0
            DO 110 I=1,NATOMS
...
************************************************************************
  ...
  160 SUM=READA(LINE,L)
     ...
     1TGEO(K,J)(2:).EQ.LINE(I:L).AND.TGEO(K,J)(1:1).EQ.'-')THEN
      ...
      WRITE(WU,'(A)')' NEGATIVE SYMBOLICS MUST BE PRECEEDED BY '
     +//' THE POSITIVE EQUIVALENT'
                  WRITE(WU,'(A)')' FAULTY SYMBOLIC:  '//SIMBOL(NVAR)
               GETGEG=.FALSE.
               RETURN
               ENDIF
               ENDIF
               IF(N.GT.1)THEN
                  ...
                  IF(TGEO(K,J)(1:1).EQ.'-')THEN
                     ...
                     IF(LOC(2,NVAR).NE.3) THEN
                       KERR=KERR+1
                       WRITE(WU,'(2A)')' ONLY DIHEDRAL SYMBOLICS CAN BE'
  ...
  170 CONTINUE
      KERR=KERR+LERR
      IF(LERR.EQ.1)THEN
         WRITE(WU,'(2A)')' THE FOLLOWING SYMBOL HAS BEEN DEFINED MORE'//
     1' THAN ONCE:',LINE(I:L)
         NERR=NERR+1
      ENDIF
      IF(N.EQ.0)THEN
         WRITE(WU,'(2A)')' THE FOLLOWING SYMBOLIC WAS NOT USED:',LINE(I:L
     1)
         NERR=NERR+1
      ENDIF
      GOTO 90
...
C#     WRITE(WU,'(2X,A,3F12.6,3I4)')ELEMNT(LABELS(I)),
C#     1(GEO(J,I),J=1,3), NA(I), NB(I), NC(I)
  200 CONTINUE
      IF(MERR.NE.0)WRITE(WU,'(I4,A)')MERR,' GEOMETRY VARIABLES WERE NOT'
     1//' DEFINED'
...
C
  ...
  220    LOC(J,K)=L
  230 CONTINUE
C#      IF(NVAR.NE.0)WRITE(WU,'(//,''    PARAMETERS TO BE OPTIMIZED'')')
      DEGREE=ASIN(1.D0)/90
      DO 240 I=1,NVAR
C#      WRITE(WU,'(2I6,F12.6)')LOC(1,I),LOC(2,I),XPARAM(I)
  240 IF(LOC(2,I).NE.1)XPARAM(I)=XPARAM(I)*DEGREE
C#      IF(NDEP.NE.0)WRITE(WU,'(//,''   SYMMETRY FUNCTIONS  '')')
C#      DO 28 I=1,NDEP
C#   28 WRITE(WU,'(3I6)')LOCPAR(I),IDEPFN(I),LOCDEP(I)
      LTXT=CHAR(MAXTXT)
      RETURN
      END
