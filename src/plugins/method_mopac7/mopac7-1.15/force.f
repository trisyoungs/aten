...
C
      ...
      IF(PRNT)WRITE(WU,'(//10X,''HEAT OF FORMATION ='',F12.6,
     1'' KCALS/MOLE'')')ESCF
      TIME2=SECOND()
      TSCF=TIME2-TIME1
      ...
      TIME3=SECOND()
      TDER=TIME3-TIME2
      IF(PRNT)WRITE(WU,'(//10X,''INTERNAL COORDINATE DERIVATIVES'',//3X,
     1''NUMBER  ATOM'',2X,''BOND'',9X,''  ANGLE'',10X,''DIHEDRAL'',/)')
      ...
      DO 40 I=1,NATOMS
         ...
         IF(PRNT)WRITE(WU,'(I6,4X,A2,F13.6,2F13.6)')
     1L,ELEMNT(LABELS(I)),(GRAD(J),J=IL,IU)
   40 CONTINUE
C   TEST SUM OF GRADIENTS
      GNORM=SQRT(DOT(GRAD,GRAD,NVAR))
      IF(PRNT)WRITE(WU,'(//10X,''GRADIENT NORM ='',F10.5)') GNORM
      IF(GNORM.LT.10.D0) GOTO 70
      IF(INDEX(KEYWRD,' LET ') .NE. 0) THEN
         WRITE(WU,'(///1X,''** GRADIENT IS VERY LARGE, BUT SINCE "LET"'',
     1'' IS USED, CALCULATION WILL CONTINUE'')')
         GOTO 90
      ENDIF
      WRITE(WU,'(///1X,''** GRADIENT IS TOO LARGE TO ALLOW '',
     1    ''FORCE MATRIX TO BE CALCULATED, (LIMIT=10) **'',//)')
   50 CONTINUE
      DO 60 I=1,NVAR
   60 SIMBOL(I)='---'
      WRITE(WU,'(//10X,'' GEOMETRY WILL BE OPTIMIZED FIRST'')')
      IF(BARTEL) THEN
         WRITE(WU,'(15X,''USING NLLSQ'')')
         CALL NLLSQ(XPARAM,NVAR)
      ELSE
         WRITE(WU,'(15X,''USING FLEPO'')')
...
C
      ...
      WRITE(WU,'(//10X,''GRADIENT NORM ='',F10.7)') GNORM
      CALL GMETRY(GEO,COORD)
   70 CONTINUE
         DO 80 J=1,NATOMS
      NAR(J)=NA(J)
...
C
  ...
  100 CONTINUE
      ...
      IF(INDEX(KEYWRD,'THERMO').NE.0 .AND.GNORM.GT.1.D0) THEN
         WRITE(WU,'(//30X,''**** WARNING ****'',//
     110X,'' GRADIENT IS VERY LARGE FOR A THERMO CALCULATION'',/
     210X,'' RESULTS ARE LIKELY TO BE INACCURATE IF THERE ARE'')')
         WRITE(WU,'(10X,'' ANY LOW-LYING VIBRATIONS (LESS THAN ABOUT ''
     1,''400CM-1)'')')
         WRITE(WU,'(10X,'' GRADIENT NORM SHOULD BE LESS THAN ABOUT '',
     1''0.2 FOR THERMO'',/10X,'' TO GIVE ACCURATE RESULTS'')')
      ENDIF
      IF(TSCF.GT.0.D0) THEN
         WRITE(WU,'(//10X,''TIME FOR SCF CALCULATION ='',F8.2)')TSCF
         WRITE(WU,'(//10X,''TIME FOR DERIVATIVES     ='',F8.2)')TDER
      ENDIF
      IF(NDEP.GT.0) THEN
         WRITE(WU,'(//10X,''SYMMETRY WAS SPECIFIED, BUT '',
     1''CANNOT BE USED HERE'')')
      ...
      IF(PRNT) THEN
        WRITE(WU,'(/9X,''ORIENTATION OF MOLECULE IN FORCE CALCULATION'')
     1')
     ...
     19X,''Y'',9X,''Z'',/)')
      ...
      DO 110 I=1,NATOMS
         ...
         L=L+1
         IF(PRNT)WRITE(WU,'(I6,7X,I3,4X,3F10.4)')
...
C$DOIT ASIS
  ...
  150 STORE(IJ)=SQRT(
     ...
     6FMATRX(((IL+2)*(IL+3))/2-1)**2))
      IF(DEBUG) THEN
         WRITE(WU,'(//10X,'' FULL FORCE MATRIX, INVOKED BY "DFORCE"'')')
         I=-NVAR
         CALL VECPRT(FMATRX,I)
      ENDIF
      IF(PRNT)THEN
         WRITE(WU,'(//10X,'' FORCE MATRIX IN MILLIDYNES/ANGSTROM'')')
  ...
  170    FREQ(I)=FREQ(I)-J*100
         IF(PRNT)THEN
            WRITE(WU,'(//10X,''TRIVIAL VIBRATIONS, SHOULD BE ZERO'')')
            WRITE(WU,'(/, F9.4,''=TX'',F9.4,''=TY'',F9.4,''=TZ'',
     1             F9.4,''=RX'',F9.4,''=RY'',F9.4,''=RZ'')')
     2(FREQ(I),I=NVIB+1,NVAR)
            WRITE(WU,'(//10X,''FORCE CONSTANTS IN MILLIDYNES/ANGSTROM''
     1,'' (= 10**5 DYNES/CM)'',/)')
            WRITE(WU,'(8F10.5)')(FREQ(I),I=1,NVIB)
C CONVERT TO WEIGHTED FMAT
            WRITE(WU,'(//10X,'' ASSOCIATED EIGENVECTORS'')')
            I=-NVAR
...
C   CONST=0.5*N*H*C/(1000*4.184)
  ...
  180 SUM=SUM+FREQ(I)
      SUM=SUM*CONST
      IF(PRNT)
     1WRITE(WU,'(//10X,'' ZERO POINT ENERGY''
...
C$DOIT VBEST
  ...
  230 CONTINUE
      IF(PRNT)THEN
         WRITE(WU,'(//3X,'' THE LAST'',I2,'' VIBRATIONS ARE THE'',
     1'' TRANSLATION AND ROTATION MODES'')')NVAR-NVIB
         WRITE(WU,'(3X,'' THE FIRST THREE OF THESE BEING TRANSLATIONS'',
     1'' IN X, Y, AND Z, RESPECTIVELY'')')
      ENDIF
      IF(PRNT.AND.LARGE)THEN
         WRITE(WU,'(//10X,'' FREQUENCIES, REDUCED MASSES AND '',
     1''VIBRATIONAL DIPOLES''/)')
         ...
         DO 240 I=1,NTO6
            ...
            IINC1=IINC1+6
            IINC2=IINC1+5
            WRITE(WU,'(3X,''I'',10I10)') (J,J=IINC1,IINC2)
            WRITE(WU,'('' FREQ(I)'',6F10.4,/)') (FREQ(J),J=IINC1,IINC2)
            WRITE(WU,'('' MASS(I)'',6F10.5,/)') (REDMAS(J),J=IINC1,IINC2
     1)
            WRITE(WU,'('' DIPX(I)'',6F10.5)') (TRDIP(1,J),J=IINC1,IINC2)
            WRITE(WU,'('' DIPY(I)'',6F10.5)') (TRDIP(2,J),J=IINC1,IINC2)
            WRITE(WU,'('' DIPZ(I)'',6F10.5,/)') (TRDIP(3,J),J=IINC1,IINC
     12)
            WRITE(WU,'('' DIPT(I)'',6F10.5)')
  ...
  250    CONTINUE
         ...
         WRITE(WU,'(/,'' DIPX(I)'',6F10.5)') (TRDIP(1,J),J=IINC1,IINC2)
         WRITE(WU,'('' DIPY(I)'',6F10.5)') (TRDIP(2,J),J=IINC1,IINC2)
         WRITE(WU,'('' DIPZ(I)'',6F10.5)') (TRDIP(3,J),J=IINC1,IINC2)
         WRITE(WU,'(/,'' DIPT(I)'',6F10.5)')
     1   (DIPT(J),J=IINC1,IINC2)
  260    CONTINUE
      ENDIF
      IF(PRNT)THEN
         WRITE(WU,'(//10X,'' NORMAL COORDINATE ANALYSIS'')')
         I=-NVAR
...
C
  ...
  290    GEO(I,J)=GEOREF(I,J)
      ...
      WRITE(WU,'(//10X,'' MASS-WEIGHTED COORDINATE ANALYSIS'')')
      I=-NVAR
      CALL MATOUT(CNORML,FREQ,NVAR,I,NVAR)
...
C
         IF(I.NE.0)THEN
            ...
            IF(I.NE.0)THEN
               I=1+READA(KEYWRD,I)
               J=NVIB-I+1
               WRITE(WU,'(//1X,''THE LOWEST'',I3,'' VIBRATIONS ARE NOT'',
     1/,'' TO BE USED IN THE THERMO CALCULATION'')')I-1
            ELSE
               WRITE(WU,'(//10X,''SYSTEM IS A TRANSITION STATE'')')
               I=2
               J=NVIB-1
            ENDIF
         ELSE
            WRITE(WU,'(//10X,''SYSTEM IS A GROUND STATE'')')
            I=1
            J=NVIB
         ENDIF
