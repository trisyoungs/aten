...
*****************************************************************
   ...
   2  FORMAT(//35X,'  Orbital energy ')
      ...
      WRITE(6,3) (I,I=1,NORBS)
   3  FORMAT(/5X,10I7)
      WRITE(6,4) (EIG(I),I=1,NORBS)
  4   FORMAT(//5X,10F7.3)
      NUMB=NELECS/2
      WRITE(6,5) NUMB
 ...
 2000 IGA(I)=0
      ...
      EPS=10.D0**(-IEPS)
      IGGW=IGGWW-1
      WRITE(6,120) IGGWW,IGGV,NIZ,IVER,EPS,ITER2,ITER3,IFULIT,
     *IPRINT,IT23
 120  FORMAT(///5X,'TOTAL NUMBER OF OCCUPIED ORBITALS INVOLVED IN THE'/
  ...
  12  III=III+1
      EG2(III)=W1
      IG2(III)=IR
      IF(IPRINT.NE.0) WRITE(6,154) W1,IR
  10  CONTINUE
   16 NUMH=NIZ
      NUMV=IVER
 ...
 1000 WRITE(6,13)
  ...
  14  FORMAT(/1X,'LEVEL ',3X,' 2   ',2X,' IR ',3X,' 3   ',2X,
     *' IR ',3X,' FINAL ',2X,' IR ',3X,' SCF')
      DO 81 II=1,III
      WRITE(6,82) NUMH,EG2(II),IG2(II),EG23(II),IG23(II),EGA(II),
     *IGA(II),EIG(NUMH)
      NUMH=NUMH+1
  81  CONTINUE
  82  FORMAT(/I3,5X,F7.3,3X,I2,2X,F7.3,3X,I2,2X,F7.3,4X,I2,4X,
     *F7.3)
      WRITE(6,83) IGGWW,IGGV,EPS,IFULIT
 ...
 359  FORMAT(/3X,'LEVEL',2X,'R.effect',3X,'ENERGY SCF',5X,'ENERGY')
      DO 357 I=1,III
      WRITE(6,358) NUMH,RIR(I),EIG(NUMH),PRIR(I)
      NUMH=NUMH+1
 357  CONTINUE
