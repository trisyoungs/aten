...
************************************************************************
      ...
      WRITE(WU,'(''  ABOUT TO ENTER FLEPO FROM PATH'')')
      TIME0=SECOND()
      CALL FLEPO(XPARAM,NVAR,FUNCT)
      WRITE(WU,'(''  OPTIMIZED VALUES OF PARAMETERS, INITIAL POINT'')')
   ...
   20    ALPARM(1,I)=XPARAM(I)
      ...
      WRITE(WU,'(1X,16(''*****'')//17X,''REACTION COORDINATE = ''
...
C
         RNORD=REACT(ILOOP)
         IF(LPARAM.GT.1) RNORD=RNORD*57.29577951D0
         WRITE(WU,'(1X,16(''*****'')//19X,''REACTION COORDINATE = ''
     1,F12.4,2X,A10,19X//1X,16(''*****''))')RNORD,TYPE(LPARAM)
C
         X3=REACT(ILOOP)
         C3=(X0**2-X1**2)*(X1-X2)-(X1**2-X2**2)*(X0-X1)
C      WRITE(WU,'(''   C3:'',F13.7)')C3
...
C
   ...
   60    IF(ABS(XPARAM(I)-ALPARM(3,I)) .GT. 0.2) GOTO 70
         GOTO 90
   70    WRITE(WU,'('' GEOMETRY TOO UNSTABLE FOR EXTRAPOLATION TO BE USED
